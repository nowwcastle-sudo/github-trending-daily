...
다국어 인터페이스, Compact Rail 탐색, 출처 결합 README 뷰어, 5개 언어 요약 파이프라인은 현재 소스에 구현되어 있습니다. 저장소에는 로컬에서 재현·검증한 45개 저장소 v1 snapshot이 포함되며, 공개 사이트의 정확한 배포 revision은 deployment manifest로 별도 확인합니다. 일반 legacy 요약, README provenance 누락, 일부 언어 요약 누락은 v1 성공 배포로 인정하지 않습니다.

아래 이미지는 2026-08-31 소스를 1440px와 390px에서 캡처한 인터페이스 예시입니다. production revision은 스크린샷이 아니라 deployment manifest로 증명합니다.
...
- **5개 언어 요약 묶음** — 모든 enrichment 저장소는 영어·한국어·중국어 간체·스페인어·일본어 요약을 함께 제공합니다. 다섯 필드의 역할과 README 근거 사실은 유지하되, 언어별 자연스러운 표현·문장 수·강조점·전체 길이는 달라도 됩니다.
- **웹·모바일 동일 요약 수준** — 반응형 레이아웃은 표시 방식만 바꾸며 모바일에서 요약 내용을 줄이거나 약화하지 않습니다.
- **Compact Rail 탐색** — 데스크톱의 64px Explore 레일은 hover 시 비모달 사이드바를, 클릭·키보드 실행 시 focus trap이 있는 고정 모달을 엽니다. 모바일은 오른쪽 가장자리 swipe로 열고 왼쪽 swipe로 닫으며, native Explore 버튼은 screen reader나 하드웨어 키보드로 접근할 때만 화면에 나타납니다.
...
- **사이트 언어에 결합된 요약** — 헤더의 사이트 언어 하나가 툴팁 요약 언어를 결정합니다. 툴팁 내부에는 별도 언어 선택이 없고, 해당 locale 요약이 불완전하면 다른 언어로 조용히 대체하지 않고 unavailable 상태를 표시합니다.
- **정확한 기간 membership** — 일간·주간·월간은 해당 기간의 유효한 rank와 gain이 있는 저장소만 보여줍니다. 전체는 세 기간의 합집합이며 총 스타만 표시하고 기간 gain·HOT·gain bar는 숨깁니다.
...
- 언어 하나 누락, 번역하면 안 되는 값의 잘못된 필드 배치·근거 불일치, 근거 부족, 스키마 오류 중 하나라도 있으면 해당 저장소와 candidate 전체가 실패합니다.
...
- 스타 추세, 연속 관측, 직전 대비 변화, 신규, 재진입 신호.
...
4. 데스크톱에서는 카드에 hover하거나 초점을 두고, 모바일에서는 탭해 헤더에서 선택한 사이트 언어의 전체 요약을 엽니다.

...
Plan 2 supersedes the original 90-minute build timing after adding release/commit preflight. The job retains `timeout-minutes: 120`, but its first executable step runs before checkout and fixes an internal origin, hard deadline at origin+115 minutes, and event deadline at origin+15 minutes. Checkout/setup/facts/events all consume that first 15 minutes. `ENRICHMENT_DEADLINE_EPOCH_MS` is fixed to `min(event_success+70m, hard-30m)`, leaving at least 30 minutes for DB, validation, commit and artifact construction plus 5 minutes runner teardown cushion; deploy/probe remain separate bounded jobs. No wall-clock or later step may reset any deadline. Do not begin an LLM attempt unless a full 60-second attempt and a 30-second admission reserve fit inside the fixed enrichment deadline. Do not sleep for a retry unless its 2s/8s delay plus the same attempt and admission reserve fit. The 60-second whole-attempt deadline includes fetch and complete response-body consumption. The separate 30-second admission reserve covers post-body JSON/schema/prose/fingerprint validation for the current response and, after the final Messages request, complete enrichment-set assembly and failure logging. Later candidate generators and deployment jobs are outside it. Normal runs use the fixed `1,000,000` input / `250,000` output policy and retry margin 12. The verified version-0 manual bootstrap has a separate immutable `bootstrap_v0_approved` policy, but the checked-in workflow deliberately selects `bootstrap_v0_pending_approval` until every implementation plan and the final security/functional gates are complete. Pending mode must fail before any paid fetch with a fixed content-free code. Bootstrap identity has two disjoint manual version-0 cases. A production-manifest 404 requires one explicit `bootstrap_source_sha` equal to the verified Pages build/hydration SHA. A valid version-0 recovery manifest requires no workflow input and uses its exact validated `sourceSha` as the hydration SHA; canonical manual SHA, manifest source SHA, and hydration SHA must match. Both require `workflow_dispatch` and the approved immutable policy. Scheduled version 0, partial configuration, a bootstrap input against any existing v0/v1 manifest, and numeric environment overrides are rejected; verified version 1 selects normal policy as defined in the state table below. Write all outputs to temp names, run the same exported coverage validator used by `validate-enrichment-coverage.mjs` against the complete active set, then atomically install; do not publish a partial queue. The CLI reports only repository/valid/compact/placeholder/applicable/N/A/missing/stale counts, never content. Success prints the numeric usage snapshot. Every failure prints one content-free structured record containing a fixed failure code and numeric usage snapshot; failures before any paid attempt report zero usage, while paid-request failures additionally include only the allowlisted request diagnostic.
...
If the production probe fails after candidate deploy, a sequential recovery job deploys the already preserved recovery artifact. It uses normal SHA/snapshot probing for version 1 and `--legacy-recovery-sha` probing for version 0. The failed verify job keeps the workflow conclusion red even when recovery succeeds. A later explicitly reauthorized `workflow_dispatch` recognizes a valid version-0 manifest, uses no workflow input, proves its `sourceSha` equals canonical manual/hydration SHA, and may retry the candidate with a null RunContext parent. A scheduled version-0 run remains blocked before RunContext, GitHub/OSS/Anthropic fetch, commit, artifact and deployment work even when the schedule variable is enabled. Recovery never force-pushes or marks the candidate snapshot successful.

- [ ] **Step 6: Execute workflow shell tests and mutation**

Run: `node --test tests/daily-refresh-workflow.test.mjs tests/pages-publication.test.mjs tests/dispatch-refresh.test.mjs tests/verify-refresh-chain.test.mjs`. Execute mutations separately and restore after each: allow SQLite into the Pages builder; reuse candidate SHA as recovery source; change failed verification to a green workflow conclusion; use main-only candidate data instead of production hydration for the next run; omit a required JavaScript file from the pre-deploy artifact; omit one active translation; skip the local HTTP probe; replace the expected receipt with current `origin/main`; accept two later-run receipt matches; let `--current-production` accept zero or two matching receipts. Add two version-0-next-run fixtures: an explicitly reauthorized `workflow_dispatch` with a valid v0 manifest, no input and exact manual/hydration identity produces a new candidate; a scheduled v0 event produces zero RunContext, external fetch, commit, artifact and deploy work. Every mutation must fail its owning test. Run `npm test` after restoration.
...
### Reference-only Deferred final Task 7: Run the one-time active-set repair and production gate only after Plans 2-5

This section is a reference runbook consumed exactly once by `2026-08-27-security-production-acceptance.md` Task 5. That Task 5 is the sole activation and dispatch owner. Do not execute this section independently or in addition to it.

**Files:**
- Generated: `data/repository-observations.sqlite`, `data/readme-state.json`, `data/repo-summaries.json`, `data/translation-sources.json`, `translations/*.json`, current public artifacts.

**Interfaces:**
- Consumes all previous tasks plus the observation DB, UI, auth, security, and repository-wide functional gates from Plans 2-5.
- Produces one first production `migration_baseline` snapshot with zero active placeholder/fallback summaries. The first production `refresh` is left to the next natural schedule; its logic is proven before dispatch by Plan 2's offline two-snapshot rehearsal.
...
3. Add RED workflow tests for the complete state table: `workflow_dispatch + manifest 404 + explicit input == verified Pages/hydration SHA → bootstrap_v0_approved`; `workflow_dispatch + valid recovery v0 manifest + no input + manual/hydration SHA == manifest sourceSha → bootstrap_v0_approved`; `schedule + recovery v0 → pending/fail before paid fetch`; `schedule + recovery v1 + GH_TRENDING_REFRESH_SCHEDULE=enabled → normal`; `workflow_dispatch + recovery v1 + no bootstrap input → normal`; and a bootstrap input with any existing v0/v1 manifest, v1 approved-mode force, partial identity, schedule value other than exact `enabled`, or numeric override → reject before paid fetch. The manual v0 activation must not disable later normal version-1 runs.
...
Fetch the currently deployed Pages SHA from the GitHub Pages build API and prove its allowlisted production file hashes match that commit. If `deployment-manifest.json` is absent, dispatch exactly once with a `bootstrap_source_sha` workflow input equal to that verified 40-hex ancestor. The workflow accepts this input only when the production manifest is 404 and rejects it on scheduled runs or after any manifest exists. If an exact valid version-0 recovery manifest exists, a later explicitly reauthorized manual retry supplies no workflow input and derives the approved immutable bootstrap identity from that manifest's `sourceSha`; it must equal canonical manual and hydration SHA. A version-1 manifest always selects the normal no-input path. The one-run boundary below still forbids reaching the recovery-retry case without a new explicit user decision.
...
Retain `$dispatch.runId` as both the expected and effective run id for every final-acceptance check. The schedule is on hold, so a different later `effectiveRunId` is a breach rather than a permitted rebind.
...
if ($verified.effectiveRunId -ne $dispatch.runId -or $verified.expectedRunId -ne $dispatch.runId) { throw 'Final acceptance cannot rebind to a later run' }
```

Require both the verifier's expected and effective runs to equal `$dispatch.runId`; any difference stops acceptance. Query the GitHub Pages deployment and Actions run via `gh api`; require successful build/deploy for that exact receipt. The verifier's bounded later-fast-forward discovery remains available for normal maintenance verification, but is forbidden in this held one-run acceptance.

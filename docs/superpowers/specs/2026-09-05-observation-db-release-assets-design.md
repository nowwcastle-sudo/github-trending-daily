...
| W1 recovery job probe (`:647`) | probe without contract → `artifactContractFromGit` | same path as `:144`; during the transition the blob fallback applied because `RECOVERY_SOURCE_SHA` could predate the pointer (removed 2026-09-06, §7) |
...
- Recovery artifact (`:156-164`): after `git archive`, `rm -f "$RECOVERY_SOURCE/data/repository-observations.sqlite"` (transition archives contained it; a no-op for pointer commits), then `resolve --source-sha "$HYDRATION_SOURCE_SHA" --expect-snapshot-id "$PARENT_SNAPSHOT_ID" --out "$RECOVERY_SOURCE/data/repository-observations.sqlite"`, then `export-contract` as today.
...
### 6.5 W1 recovery job
The probe at `:647` runs without a contract and therefore resolves through `artifactContractFromGit` (§6.3). During the transition `RECOVERY_SOURCE_SHA` could predate the pointer and `resolve` served the blob; since 2026-09-06 a blob-only `RECOVERY_SOURCE_SHA` fails closed (see §8, rollback row).
...
`approvedGeneratedCommitPath`: add `data/observation-db.pointer.json`. During the transition it also kept `data/repository-observations.sqlite` accepted because the transition commit deleted it (diff-tree name lists do not distinguish add from delete); the follow-up PR removed the sqlite name on 2026-09-06 (see §7).
...
"Clean run" means the deployed manifest's `sourceSha` is a pointer commit, not merely a green workflow. After two clean runs a follow-up PR deletes the blob branch of `resolve`, the transition `git rm --cached`, and the sqlite name in `approvedGeneratedCommitPath` (same pattern as the probe-tolerance removal). Done 2026-09-06 after clean runs 33998950309 (dispatch, pointer commit e82eced) and 34020238117 (schedule, pointer commit 6cc335a deployed): `resolve` now fails closed on a blob-only commit (`pre-transition blob without a pointer`, exit 1), the promote step no longer stages a deletion, and the chain verifier no longer accepts the sqlite name. Consequence: run 33998950309's receipt is no longer chain-verifiable with `--expected-run-id` (its generated commit deleted the sqlite); production has moved on to later commits, so nothing live depends on it.
...
| production rolled back to a pre-transition artifact (re-running the `deploy` job of a run older than e82eced) | the next W1 stops at the state check: `resolve --check` on a blob-only `sourceSha` exits 1 (`pre-transition blob without a pointer`) | dispatch `deploy-current-pages.yml` (deploys `origin/main`, a pointer commit); W2 is unaffected because it checks HEAD |
...
- `verify-refresh-chain.test.mjs`: a diff-tree list with the pointer passes and one naming the sqlite is rejected as non-generated (it passed during the transition, until 2026-09-06); fake git learns `show <sha>:data/observation-db.pointer.json`; probe resolver stubbed via `OBSERVATION_DB_RESOLVER_SCRIPT`.
...
5. After two clean runs: follow-up PR removing the transition fallback (§7). Done 2026-09-06.

...
| `frame-src` | `https://github-trending-nowwcastle.firebaseapp.com` | `signInWithPopup` (`firebase-client.js:229`) also installs a hidden `https://<authDomain>/__/auth/iframe`. `authDomain` is pinned to `github-trending-nowwcastle.firebaseapp.com` (`firebase-client.js:98`), so the exact host — not a `*.firebaseapp.com` wildcard — is used |
...
4. **Nonce/hash corollary.** This policy carries no nonce and no hash, so `'unsafe-inline'` in `script-src` is honoured. Any future task that adds a nonce or hash to `script-src` silently voids `'unsafe-inline'` and breaks both inline scripts (index.html:669 and :1630 as of this writing) — check for this before hardening `script-src` further. `form-action 'self'` also blocks `signInWithRedirect`; adding a redirect fallback requires `form-action 'self' https://github-trending-nowwcastle.firebaseapp.com`.
...
**Amendment (2026-09-04, Task 7 fix round, review F1/F2/F3/F7).** `shortcutSuppressed` gains a first check — `event.repeat || event.isComposing || event.key === "Process"` — so a held key and IME composition no longer re-fire the accelerators; the listener also reads `sidebar.dataset.openMode==="modal"` before acting: `/` returns without `preventDefault()` while the panel is already modal instead of focusing the inert `#q`, and a group letter calls `setSidebarGroup(group)` in place instead of being silently eaten by `openSidebar`'s already-modal early return. Because shortcuts match `event.key` (not `event.code`), they are unavailable while a non-Latin IME layout (Hangul, kana) is active — an accepted limitation (F7), since every group stays reachable by rail button, mobile trigger, and `#sidebarGroupSeg`.

**Amendment (2026-09-04, consolidated fix round, RED TEAM 1 M4/H3).** CapsLock no longer matters: the listener reads `SIDEBAR_SHORTCUT_GROUPS[event.key.toLowerCase()]`, and lowercasing is applied only to single-character keys so the `"Process"` and named-key guards keep matching their exact spellings. A non-Latin IME layout still disables the accelerators, and that remains accepted — but the F7 justification above is now actually true in every viewport: RED TEAM 1 H3 found that between 0 and 720 px with a fine pointer the rail was `display:none`, the edge gesture was disarmed and `#mobileNavToggle` was a 1 px skip link, leaving no visible opener at all. `#mobileNavToggle` is now rendered as a real 44 px header button wherever `sidebarMobileAccessMedia` matches, so an IME-typing user in a half-screen window always has a pointer route in.

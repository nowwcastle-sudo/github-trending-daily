...
8. 현재 대체 provider key 발급 중이므로 모든 Anthropic·대체 LLM 실호출은 일시 보류한다. producer·validator·UI·테스트 구현은 계속하되 key가 준비됐다는 후속 지시 전에는 호출하지 않는다.
9. controlled workflow와 Pages 배포는 1~7단계 결과 보고 뒤 사용자 확인을 받은 후 8단계에서만 수행한다. 그 전에는 workflow dispatch와 deploy를 실행하지 않는다.
...
5. **Sonnet 5 producer·coverage gates — 코드 완료, 실호출 보류**: key 준비 전 실 LLM 호출은 0회로 유지한다.
6. **Focused/full/browser/screenshots — 진행 중**: Python full과 focused Node는 green. 390/720/1200/1440, 5 locale, hover/click/focus/Escape, light/dark, tooltip, README legacy failure UI, export privacy, Atom/membership을 실브라우저에서 확인했다. 최종 full Node와 adversarial/mutation을 남겼다.
7. **Secret·remote drift·commit/push·결과 보고 — 대기**: 전체 검증 green 뒤 staged/working-tree secret scan, fetch, diff, commit/push를 수행하고 workflow 이전 결과를 보고한다.
8. **사용자 확인 후 controlled workflow·Pages — 승인 대기**: 정확히 한 번의 controlled refresh와 Pages deployment, manifest·usage·bot commit·snapshot·observation·production full census를 묶어 검증한다.

...
- workflow는 GitHub-hosted `prepare` → Windows self-hosted `enrich` → GitHub-hosted `publish`로 분리한다. self-hosted runner는 tracked checkout·DB·page·commit·Pages 권한이 없고 네 개의 allowlisted enrichment 파일만 내보낸다. manual bootstrap gate는 승인 상태이고 schedule은 계속 hold한다. 일곱 controlled run 모두 publish 전 fail-closed로 종료됐다.
...
6. **검증·commit/push·controlled workflow·Pages — 일곱 번째 dispatch의 invariant-specific correction repair 검증 완료**: 기존 390/720/1200/1440, 5 locale, hover/click/focus/Escape, light/dark, tooltip, README legacy failure UI, export privacy, Atom/membership 실브라우저 검증을 보존한다. 첫 여섯 run의 원인과 frozen evidence는 위 이력대로 유지한다. 일곱 번째 run `33358622337`은 interactive runner에서 여러 repository의 실제 Claude enrichment를 수행한 뒤 `Summary bundle invariant is absent from README: Python 3.13+`로 fail-closed 종료됐다. source README에는 `**Python**: 3.13+`가 있어 `Python 3.13+`는 raw Markdown exact substring이 아니었고, 기존 class-only correction에는 거부된 값과 literal-substring 규칙이 없었다. exact completed model calls와 tokens는 usage receipt가 없어 `unknown`이며 DB·commit·Pages·probe는 0이다. 해당 frozen input은 snapshot `20260831045325-d54f11d3bb736e1b`, repositories `50`, `sourceSetSha256=66389f013c5bf3adcd387849874eb68e8f0cfe39fbdf7eb7190ce73b23b929f5`, `runContextSha256=1d766777c37b015af179adfa375efbf983e772f98b168b20d31c0210f6d320d0`, `factsSha256=1d2213ac308efeb1ac44cd16de6649a8fe178cfc402aabd1d910fb7ab3e3f187`다. 새 RED는 같은 invalid invariant가 두 번 이어진 뒤 exact source substring `3.13+`를 요구하며 기존 코드에서 실패했고, 거부된 값을 JSON-escaped feedback으로 전달하고 raw README exact literal substring 또는 omit만 허용해 green이 됐다. 전용 피드백을 무력화한 변이는 RED를 다시 실패시켰다. 최종 local 기준선은 Node 518개(506 pass, 12 intentional skip), Python 146 pass, Firestore Rules 9 pass다.
...
- 두 번째·세 번째·여섯 번째·일곱 번째 run에서 실제 Claude enrichment 요청은 시작됐지만 validator 실패로 완성 bundle과 usage receipt가 남지 않아 token count는 `unknown`이다. production DB·commit·Pages publication은 일곱 run 모두 0회다.
...
- controlled workflow `33337114759`, `33338119441`, `33340906781`, `33353784160`, `33355529363`, `33356905445`, `33358622337`은 각각 prepare test, enrichment evidence gate, enrichment invariant gate, runner Worker handoff, reboot 후 runner handoff, Spanish placeholder gate, raw README invariant substring gate에서 publish 전에 종료됐다. 필요한 후속 dispatch는 승인됐지만 cap 상향·direct API·새 유료 provider는 승인되지 않았다.

...
9. controlled workflow와 Pages 배포는 로컬 검증·비밀값 검사·remote drift·runner online/OAuth preflight가 모두 통과한 뒤 6단계 말미에 수행한다. 사용자가 첫 번째·두 번째·세 번째 dispatch를 각각 승인했고 세 run 모두 publish 전에 실패로 종료됐다. 네 번째 dispatch는 새 사용자 승인 없이는 금지한다.
...
- workflow는 GitHub-hosted `prepare` → Windows self-hosted `enrich` → GitHub-hosted `publish`로 분리한다. self-hosted runner는 tracked checkout·DB·page·commit·Pages 권한이 없고 네 개의 allowlisted enrichment 파일만 내보낸다. manual bootstrap gate는 승인 상태이고 schedule은 계속 hold한다. 세 controlled run 모두 publish 전 fail-closed로 종료됐다.
...
6. **검증·commit/push·controlled workflow·Pages — 세 번째 dispatch 실패의 local repair 검증 완료, 네 번째 승인 대기**: 기존 390/720/1200/1440, 5 locale, hover/click/focus/Escape, light/dark, tooltip, README legacy failure UI, export privacy, Atom/membership 실브라우저 검증을 보존한다. 첫 run `33337114759`는 `prepare`의 Windows/Linux 이식성 assertion 3건으로 실패해 enrich·DB·commit·Pages와 model call이 모두 0이었다. 두 번째 run `33338119441`은 self-hosted enrichment를 실제 시작했지만 `Summary bundle README evidence heading is invalid for goal`에서 실패했고, prompt schema v2가 line range만 받아 heading을 파생하도록 고쳤다. 세 번째 run `33340906781`은 clean main `d3c8fdc3f61ca816104985d7bda40c9e725954be`에서 prepare·50 repository frozen facts·complete events까지 성공하고 2026-08-30T23:11:43Z부터 Claude enrichment를 실제 실행했지만 `Summary bundle invariant is missing from en.fit`로 23:21:11Z에 실패했다. source-bound frozen input은 snapshot `20260830230613-a2107b023cb5e066`, `sourceSetSha256=068adca6af705ff15f65b305fb75c8b8a95150e4e08728beed2a888a06e5268e`, `runContextSha256=603a6dc89f9ca406bc088ae2117635ff2ddfa2e464f7fb65eca285e5cc0aa2ed`, `factsSha256=cdc32d12d8f7a74cef6f1ab82c4a3bc501dc475043a3591b8a9e3090eae11230`, `eventsCompleteSetSha256=238e90fc964f3fe206af38c7d136fd57238938af00730cbc9811802066f81e83`다. enrichment output·usage receipt·DB·commit·Pages·probe는 생성·실행되지 않아 exact model calls와 tokens는 `unknown`이다. prompt schema v3는 모델 invariant를 `kind`/`value`로 제한하고 field 위치를 실제 5-locale summary에서 파생·대조하며, 영구 실패 후 다른 worker가 새 repository를 호출하지 못하게 한다. 실제 세 번째 frozen 50개를 model call 없이 planner로 재생해 50/50 line-only evidence, 50/50 derived invariant schema, 총 input 3,540,630 bytes, 최대 request 1,495,614 bytes를 확인했다. 최종 local 기준선은 Node 516개(504 pass, 12 intentional skip), Python 146 pass다. 네 번째 dispatch는 새 사용자 승인 전까지 금지한다.
...
- 두 번째와 세 번째 run에서 실제 Claude enrichment 요청은 시작됐지만 validator 실패로 완성 bundle과 usage receipt가 남지 않아 token count는 `unknown`이다. production DB·commit·Pages publication은 세 run 모두 0회다.
...
- controlled workflow `33337114759`, `33338119441`, `33340906781`은 각각 prepare test, enrichment evidence gate, enrichment invariant gate에서 실패했다. 네 번째 dispatch·cap 상향·direct API·새 유료 provider는 별도 승인 없이는 금지한다.

...
다음 코드 라운드를 `N1 관심 없음` 하나로 작게 시작할지, 같은 사이드바 상태를 공유하는 `N1 관심 없음 + N2 명시적 정렬`을 두 개의 독립 커밋으로 한 라운드에 구현할지 사용자에게 확인한다.

권고는 **N1을 먼저 완성하고 같은 라운드에서 N2를 별도 커밋으로 이어가는 것**이다. 둘 다 기존 공개 데이터와 브라우저 상태만 사용하고 운영비가 없지만, N1은 개인 상태·복구 UX를 추가하고 N2는 URL 호환성과 정렬 불변식을 추가하므로 테스트와 커밋을 합치지 않는다.

사용자가 N1~N5 전체를 아직 일괄 구현 승인한 것은 아니다. 직전 조사에서 `I1 I2 I3`만 명시적으로 채택했고, `N1~N5`는 “다음 순서 권고”였다. 새 세션은 범위를 확인한 뒤 승인된 후보만 구현한다.
...
직전 조사에서 합의에 사용한 전체 순서는 다음과 같다. I1~I3만 구현 완료다.
...
1. 사용자에게 N1 단독 또는 N1+N2 순차 구현 범위를 먼저 확인한다.
2. 기능별 RED 테스트를 먼저 만든다.
3. N1과 N2는 한 파일에서 함께 수정돼도 커밋과 검증 증거를 분리한다.
4. 브라우저 localStorage·URL 상태에는 허용값과 크기 제한을 둔다.
5. 모바일 390px, 중간 폭 720px, desktop 1200px·1440px를 실제 브라우저로 측정한다.
6. sidebar를 열 때 tooltip을 닫고, README를 열 때 sidebar와 tooltip을 닫는 overlay coordinator를 보존한다.
7. 카드 높이와 폭을 늘리는 permanent control을 추가하지 않는다.
8. 전체 `npm test`, 의도적 변이, staged added-content 비밀값 스캔 뒤 커밋·push한다.
9. Pages build와 deploy를 모두 확인하고 production에서 다시 검증한다.
10. production 확인 뒤에만 두 README의 현재 기능·로드맵을 함께 갱신한다.
11. 한 라운드가 끝나면 다음 후보를 자동 구현하지 말고 사용자 확인을 받는다.
